...
erDiagram
    ...
    users ||--o{ homebrew_entries : writes
...
| `homebrew_entries.owner_id` | `users.id` | user-authored catalog entries |
| `homebrew_entries.campaign_id` | `campaigns.id` | the campaign a `campaign`-visibility entry is shared with |
...
| `homebrew_entries` | A catalog entry one user wrote (issue #132). | The same data as a `content_entries` row — same content type, same validation, same renderer — but owned, so it is per user and editable where pack content is neither. FK `owner_id` (indexed); `schema_id` is **not** a foreign key, matching `content_packs`, because schemas are per user and homebrew may outlive the copy it was written against. `visibility` ∈ private/campaign/public, enforced server-side in one shared filter whose campaign arm checks **membership** rather than merely that `campaign_id` is set. `campaign_id` survives a visibility change, so an entry set back to private remembers where it was shared. `forked_from` records the source as `"<source>:<entry_id>"`. **Unique** `(owner_id, schema_id, content_type, entry_id)` — two users may each own a `hellfire-blast`. Deleting a row leaves referencing characters intact: references are soft, so the sheet marks the entry missing rather than losing it. |
